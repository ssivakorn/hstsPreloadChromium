...
if __name__ == "__main__":
    ...
    # Modified so this can get full url
    #hits = hsts.lookup(args.entries)
    entries = []
    for url in args.entries:
        try:
            urlelem = tldextract.extract(url)
            entry = ""

            if urlelem.subdomain:
                entry += "%s." % urlelem.subdomain
            if urlelem.domain:
                entry += "%s" % urlelem.domain
            if urlelem.suffix:
                entry += ".%s" % urlelem.suffix

            if entry:
                entries.append(entry)
        except:
            print("ERROR. URL not able to be parsed -- %s" % url,
                  file=sys.stderr)



    hits = hsts.lookup(entries)
    for hit in hits:
        print("%s" % hit)
